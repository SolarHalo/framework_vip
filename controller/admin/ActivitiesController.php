...
//	    $name = iconv( 'UTF-8', 'gb2312' , $_POST['activities_name']); 
   ...
   public function updateactivity($param){
   	    $this->getSmarty();
	    require_once SERVICE.DS.'admin'.DS.'Activities.class.php';
		$activitiesService = new Activities($this->getDB()); 
	    $activities = $activitiesService->activityByid($param[0]);
		$defaultId = $activitiesService->finddefaultImgIdByid($param[0]);
		 
		$activity = $activities[0];
		$this->smarty->assign("activity",$activity);  
		$this->smarty->assign("defaultid",$defaultId->id);  
		$this->smarty->assign("activities",$activities);  
		$this->smarty->display("admin/activities_update.tpl");
   }
  public function delactivity($param){
   	     
	    require_once SERVICE.DS.'admin'.DS.'Activities.class.php';
		$activitiesService = new Activities($this->getDB()); 
		$activitiesService->delactivity($param[0]); 
	    $this->index();
   }
...
//	    $name = iconv( 'UTF-8', 'gb2312' , $_POST['activities_name']); 
	    ...
	    require_once SERVICE.DS.'admin'.DS.'Activities.class.php';
	    $activities = new Activities($this->getDB()); 
	    $activities->updateActivity($data,$id);
	    $activities->delFilesByacId($id);
	    foreach ($files as $file) {
	    	$imgdata = array("activ_id"=>$id,"imgname"=>$file,"default_show"=>0);
	    	if(strcmp($file,$defaultimg) == 0){
	    		$imgdata['default_show'] = 1;
